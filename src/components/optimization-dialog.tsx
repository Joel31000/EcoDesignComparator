...
export function OptimizationDialog({ simulationState, calculationResults, onStateChange, isMenuItem = false }: OptimizationDialogProps) {
  ...
  const handleOpenTrigger = (e: React.MouseEvent<Element, MouseEvent>) => {
    setResult(null);
    setMaxSurcout('');
    if (isEcoCheaper) {
      e.preventDefault();
  ...
  const triggerProps = isMenuItem
    ? { onSelect: handleOpenTrigger, children: <><Sparkles className="mr-2" />Optimisation du design</> }
    : { 
        ...
        children: <><Sparkles className="mr-2" />Optimisation du design</>
      };
